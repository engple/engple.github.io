...
잉플은 영어 학습 사이트입니다. UI는 **공책과 형광펜** 메타포를 따릅니다.

- **인디고 프라이머리**: 신뢰감 있는 교육 브랜드 톤. 활성 상태, 학습 UI(연습문제 번호, FAQ 인덱스, TOC 활성 항목)에 사용.
- **앰버 형광펜 액센트**: 본문 `strong`의 하이라이터, 제목 아래 마커 바, 브랜드 도트.
  "핵심 표현에 형광펜을 긋는" 학습 경험을 시각화합니다.
- **웜 페이퍼 배경**: 라이트 테마 배경(`--color-background`)은 종이 톤(#f7f5f0).
- **북시 디스플레이 서체**(`--font-display`): 페이지 H1과 브랜드 로고에만 사용하는
  시스템 serif 스택 — 사전/교재의 인상을 주면서 웹폰트 로딩 비용은 0입니다.
...
| 토큰                     | 라이트                      | 다크                    | 용도                                                   |
| ------------------------ | --------------------------- | ----------------------- | ------------------------------------------------------ |
| `--color-primary`        | `#3b5bdb`                   | `#748ffc`               | 브랜드 인디고(링크 하이라이트, 발음 버튼, 활성 상태)   |
...
`--font-display`(시스템 serif 스택)는 **페이지 H1(홈 히어로, 포스트 제목)과 내비게이션
브랜드에만** 사용합니다. 본문·카드·버튼 등 UI 텍스트에는 사용하지 않습니다.
