...
class TestExprPath:
    ...
    def test_get_expr_path(self):
        ...
        assert (
            result1.file_path.resolve()
            == Path("../src/posts/blog/in-english/398.get-rid-of.md").resolve()
        )
        ...
        assert (
            result2.file_path.resolve()
            == Path("../src/posts/blog/vocab-1/043.on-time.md").resolve()
        )
        ...
        assert (
            result3.file_path.resolve()
            == Path("../src/posts/blog/season-1/가능성이-높아-영어표현.md").resolve()
        )
