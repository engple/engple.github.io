"""Context detection utilities for markdown and HTML content."""


def is_inside_existing_link(text: str, position: int) -> bool:
...
def is_html_context(text: str, position: int) -> bool:
...
def should_skip_context(text: str, start: int, end: int) -> bool:
    ...
    # Skip code blocks
    if _is_in_code_block(text, start):
        return True

    # Skip headers
    if _is_in_header(text, start):
        return True

    # Skip YAML front matter
    if _is_in_yaml_frontmatter(text, start):
        return True

    # Skip HTML comments
    if _is_in_html_comment(text, start):
        return True

    # Skip inside HTML tags
    if _is_in_html_tag(text, start):
        return True

    return False


def _is_in_html_tag(text: str, position: int) -> bool:
...
def _is_in_header(text: str, position: int) -> bool:
...
def _is_in_yaml_frontmatter(text: str, position: int) -> bool:
...
def _is_in_html_comment(text: str, position: int) -> bool:
